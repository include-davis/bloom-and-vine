  ...
  return javascriptStringify.stringify(value, (value, _indent, next) => {
    ...
    if (valueType === 'function' && (value.__function_serializer__ || value.__recipe__)) {
      ...
      try {
        ...
        return `${hashedImportName}(${args.map(arg => stringifyExports(functionSerializationImports, arg, unusedCompositionRegex, key, exportLookup)).join(',')})`;
      } catch (err) {
        console.error(err);
        throw new Error('Invalid function serialization params');
      }
...
function serializeVanillaModule(cssImports, exports, unusedCompositionRegex) {
  ...
  const moduleExports = Object.keys(exports).map(key => {
    const serializedExport = stringifyExports(functionSerializationImports, exports[key], unusedCompositionRegex, key === 'default' ? defaultExportName : key, exportLookup);
    if (key === 'default') {
      return [`var ${defaultExportName} = ${serializedExport};`, `export default ${defaultExportName};`].join('\n');
    }
    return `export var ${key} = ${serializedExport};`;
  });
  const outputCode = [...cssImports, ...functionSerializationImports, ...moduleExports];
